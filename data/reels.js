...
function buildPageHeading(dict){
  ...
  h1.innerHTML =
    `${dict.reels.headingPlain}<span class="grad-text">${dict.reels.headingGrad}</span>`;
}
...
function openModal(reel){
  ...
  if (reel.videoUrl && isPlayableVideoUrl(reel.videoUrl)){
    modalVideoWrap.innerHTML =
      `<video class="modal-video" src="${reel.videoUrl}" controls playsinline></video>`;
  } else if (reel.videoUrl){
    modalVideoWrap.innerHTML =
      `<a class="modal-video-link" href="${reel.videoUrl}" target="_blank" rel="noopener">▶ Watch video ↗</a>`;
  } else {
    modalVideoWrap.innerHTML = '';
  }

  modalStoryboard.innerHTML = (reel.storyboard || [])
    .map((step, i) => `
      <li>
        <span class="sb-num">${i+1}</span>
        <span>${L(step, currentLang)}</span>
      </li>
    `)
    .join('') ||
    `<li><span>${r.empty || 'No storyboard steps added yet.'}</span></li>`;
...
function renderCard(reel, lang){
  ...
  const btnLabel =
    (dict.reels && dict.reels.storyboardBtn) ||
    'View Storyboard →';

  const gradient =
    BRAND_GRADIENTS[reel.brand] || DEFAULT_GRADIENT;
  ...
  // No autoplay here.
  const videoHtml =
    (reel.videoUrl && isPlayableVideoUrl(reel.videoUrl))
      ? `<video class="reel-video" src="${reel.videoUrl}" muted loop playsinline preload="metadata"></video>`
      : '';
  ...
  card.innerHTML = `
    ...
    <div class="reel-bottom">
      <div class="reel-title">
        ${L(reel.title, lang) || 'Untitled idea'}
      </div>

      <button class="storyboard-btn">
        ${btnLabel}
      </button>
    </div>
  `;
...
function renderPlaceholder(lang){
  ...
  const text =
    (dict.reels && dict.reels.empty) ||
    'No Reels & Stories ideas added yet — check back soon.';
...
function renderReels(lang){
  ...
  if(reelsData.length === 0){
    ...
    reelsData.forEach(reel=>{
      grid.appendChild(renderCard(reel, lang));
    });
  }
}
...
const FALLBACK_REELS = [{
  ...
  title: {
    en: 'Mazaq Day 2 — Study Mood On',
    ar: 'مذاق — اليوم التاني: مود المذاكرة اشتغل'
  },
  ...
  storyboard: [
    {
      en: 'Late-night study/work scene with the person focused on the laptop and Mazaq beside them.',
      ar: 'مشهد مذاكرة أو شغل بالليل، والشخص مركز في اللابتوب ومذاق جنبه.'
    },

    {
      en: 'The person keeps studying while the voice-over says: “وراكم مذاكرة كتير؟”',
      ar: 'الشخص مكمل مذاكرة، والـVoice-over بيقول: «وراكم مذاكرة كتير؟»'
    },

    {
      en: 'A subtle camera movement reveals the Mazaq coffee, connecting it naturally to the study moment.',
      ar: 'حركة كاميرا بسيطة تكشف قهوة مذاق وتربطها بالمود بشكل طبيعي.'
    },

    {
      en: 'The voice-over continues: “مذاق بيظبط يومك.”',
      ar: 'الـVoice-over بيكمل: «مذاق بيظبط يومك.»'
    }
  ],
...
// Always show the current reel first.
...
i18nData = {
  en: {
    ...
    reels: {
      ...
      headingPlain: 'Reels & Stories, ',
      headingGrad: 'ready to film.',
      lede: 'Short vertical video ideas (9:16) — tap a card to see the full storyboard and what makes the idea work.',
      back: '← Back to portfolio',
      storyboardBtn: 'View Storyboard →',
      storyboardLabel: 'Storyboard',
      whyLabel: 'Why This Works',
      empty: 'No Reels & Stories ideas added yet — check back soon.',
      fine: '© 2026 Yara Ayman Hussein'
    }
  },
  ...
  ar: {
    ...
    reels: {
      eyebrow: 'أفكار محتوى',
      headingPlain: 'ريلز وستوريز، ',
      headingGrad: 'جاهزين للتصوير.',
      lede: 'أفكار فيديوهات عمودية قصيرة (9:16) — دوسي على أي كارت علشان تشوفي الستوري بورد كامل وإيه اللي بيخلي الفكرة تشتغل.',
      back: '← رجوع للبورتفوليو',
      storyboardBtn: 'شوف الستوري بورد ←',
      storyboardLabel: 'الستوري بورد',
      whyLabel: 'ليه ده شغال؟',
      empty: 'لسه مفيش أفكار ريلز وستوريز متضافة — تابعينا قريب.',
      fine: '© 2026 يارا أيمن حسين'
    }
  }
};
